...
def summarize_flight_information(db: Database, flight_data: Dict) -> str:
    ...
    origin = get_airports_name(db, origin_code)['AIRPORT_NAME']
    destination_code = flight_data.get('destinationCode', '')
    destination = get_airports_name(db, destination_code)['AIRPORT_NAME']
    ...
    price = format(int(float(flight_data.get('price', 0))),',')
    airline = get_airline_data(db, flight_data.get('airlineCode', '알 수 없음'))['carrierName']
